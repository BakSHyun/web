...
class SMS
{
	...
	function reportSMS($sms_id,$sms_pwd,$firstdate,$seconddate,$mode)
	{

		$hash_value=md5($sms_id.$sms_pwd);//해쉬값을 생성한다.
		//통큰아이 웹서비스 웹 접속 포인트를 설정한다.
		$soapclient = new nusoap_client('http://reportservice.tongkni.co.kr/reportSMS/ServiceReportSMS.asmx?wsdl',true);
		...
		$result = $soapclient->call('reportSMS',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.
		return $result['reportSMSResult'];
	}

	function CountreportSMS($sms_id,$sms_pwd,$askmonth)
	{
		...
		$result = $soapclient->call('CountreportSMS',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.
		return $result['CountreportSMSResult'];
	}


	function ReservereportSMS($sms_id,$sms_pwd)
	{
		...
		$result = $soapclient->call('ReservereportSMS',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.

		return $result['ReservereportSMSResult'];
	}

	function RemainSMS($sms_id,$sms_pwd)
	{
		...
		$result = $soapclient->call('RemainSMS',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.

		return $result['RemainSMSResult'];
	}
