...
$sql = "SELECT id,name,nicname,birth,sex,email,post,address1,address2,area,tel,htel,office,opost,oaddress1,oaddress2,otel,etc,infoopen,infomail,admail,regdate,level,logindate FROM ammember where level >0 order by regdate desc";
...
<?
 ...
 if($FIELDINFO[office][state] != 0){
  ...
  echo "<td align='center'>광고메일수신</td>";
  }
  if($FIELDINFO[admail][state] != 0){
  echo "<td align='center'>정보공개</td>";
  }
  echo "<td align='center'>가입일</td>";
  echo "<td align='center'>최근로그인</td>";
  echo "<td align='center'>레벨</td>";
	echo "</tr>";
	echo "</table>";

	echo "<table border='1'>";
	while ($row = mysql_fetch_row($stmt)){
		$level= $levelinfo[$row[22]];
	echo "<tr>";
	if($FIELDINFO[id][state] != 0){
