...
class SMS
{
	function SendSMS($sms_id,$sms_pwd,$snd_number,$rcv_number,$sms_content)
	{
		...
		if(preg_match("'^[0-9,]+$'",$snd_number.$rcv_number)){
			$soapclient = new nusoap_client('http://webservice.tongkni.co.kr/sms.2/ServiceSMS.asmx?WSDL',true);
	...
	{
		$hash_value=md5($sms_id.$sms_pwd);
		$soapclient = new nusoap_client('http://webservice.tongkni.co.kr/sms.3/ServiceSMS.asmx?WSDL',true);
		$soapclient->xml_encoding = "UTF-8";
		$soapclient->soap_defencoding = "UTF-8";
		$soapclient->decode_utf8 = false;
		$parameters[] = array(
