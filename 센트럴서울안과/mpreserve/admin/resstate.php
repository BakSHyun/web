...
$dbobj->addfield("etc5",$etc5);
$dbobj->addfield("resdate",$resdate);
$dbobj->addfield("restime",$restime);
...
if($sendsms == "Y" && ($resstate == "1" || $resstate == "9"))
{
	...
	{
		$snd_number = $adm_sql[adm_phone];    //지점별관리자 번호가 없을경우 병원전화번호로 회원에게 sms를 발송한다 .
		} else {
		$snd_number =  $config[smsmail_jijum_phonenum];		//지점별 관리자가 있는경우 지점별관리자의 핸드폰번호로 sms를 발송한다.
	}

	$msg = "[".$adm_sql[adm_meta_title]."] " . $row[name]."님은 ";

	if($resstate == "1")
		$msg .= $wyear."년 ".$wmonth."월 ".$wday."일 ".$rtime." 예약이 확인되었습니다.";
	else if($resstate == "9")
