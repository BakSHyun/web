<?php
	...
	* 작성일자:2011년 06월 30일
	* 작성목적: 본 프로그램은 통큰아이 문자메세지에서 php 웹프로그램 관련 예제입다.
	*           본 예제에서는 보내는 사람의 핸드폰번호, 받는사람 핸드폰번호,전송내용 mms_main.php에서
	*           전송 받아 실제 처리하는  mms 모듈입니다.
	*/
...
function get_file($getfilename)
{
	if($fp = @fopen($getfilename,r))
	{
		$content = fread($fp,filesize($getfilename));
		fclose($fp);
	}
	return $content;
}
...
class MMS
{
	function SendMMS1($mms_id,$mms_pwd,$snd_number,$rcv_number,$mms_title,$mms_content,$filepath,$filename)
	{
		$snd_number=str_replace("-","",$snd_number);
		$rcv_number=str_replace("-","",$rcv_number);
		$hash_value=md5($mms_id.$mms_pwd.$rcv_number);//해쉬값을 생성한다.
		if(preg_match("'^[0-9,]+$'",$snd_number.$rcv_number)){

			$filedata = base64_encode(get_file($filepath.$filename));
			$soapclient = new nusoap_client('http://mmsservice.tongkni.co.kr/mms.1/ServiceMMS.asmx?WSDL',true);
			$parameters[] = array(
				    'mmsID'=>$mms_id,
			...
			$result = $soapclient->call('SendMMS1',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.

			return $result['SendMMS1Result'];
 		}
		else
		{
			return "-50";
	...
	function SendMMS2($mms_id,$mms_pwd,$snd_number,$rcv_number,$mms_title,$mms_content,$filepath1,$filename1,$filepath2,$filename2)
	{
		$snd_number=str_replace("-","",$snd_number);
		$rcv_number=str_replace("-","",$rcv_number);
		$hash_value=md5($mms_id.$mms_pwd.$rcv_number);//해쉬값을 생성한다.
		if(preg_match("'^[0-9,]+$'",$snd_number.$rcv_number)){
			...
			$filedata1 = base64_encode(get_file($filepath1.$filename1));
			$filedata2 = base64_encode(get_file($filepath2.$filename2));
			$soapclient = new nusoap_client('http://mmsservice.tongkni.co.kr/mms.1/ServiceMMS.asmx?WSDL',true);
			$parameters[] = array(
				    'mmsID'=>$mms_id,
				    'hashValue'=>$hash_value,
			        ...
			        'imagefile1'=>$filedata1,
					'imagename2'=>$filename2,
			        'imagefile2'=>$filedata2
			);
			$result = $soapclient->call('SendMMS2',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.

			return $result['SendMMS2Result'];
 		}
		else
		{
			return "-50";
	...
	function SendMMSReserve1($mms_id,$mms_pwd,$snd_number,$rcv_number,$mms_title,$mms_content,$filepath,$filename,$reservedate,$reservetime,$userdefine)
	{
		$snd_number=str_replace("-","",$snd_number);
		$rcv_number=str_replace("-","",$rcv_number);
		$hash_value=md5($mms_id.$mms_pwd.$rcv_number);//해쉬값을 생성한다.
		if(preg_match("'^[0-9,]+$'",$snd_number.$rcv_number)){
			...
			$result = $soapclient->call('SendMMSReserve1',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.

			return $result['SendMMSReserve1Result'];
 		}
		else
		{
			return "-50";
	...
	function SendMMSReserve2($mms_id,$mms_pwd,$snd_number,$rcv_number,$mms_title,$mms_content,$filepath1,$filename1,$filepath2,$filename2,$reservedate,$reservetime,$userdefine)
	{
		$snd_number=str_replace("-","",$snd_number);
		$rcv_number=str_replace("-","",$rcv_number);
		$hash_value=md5($mms_id.$mms_pwd.$rcv_number);//해쉬값을 생성한다.
		if(preg_match("'^[0-9,]+$'",$snd_number.$rcv_number)){
			...
			$result = $soapclient->call('SendMMSReserve2',$parameters); //실제적으로 전송을 실시하고 결과 값을 받습니다.

			return $result['SendMMSReserve2Result'];
 		}
		else
		{
			return "-50";
	...
	function GetRemainCount($mms_id,$mms_pwd)
        {
                $hash_value=md5($mms_id.$mms_pwd);
                $soapclient = new nusoap_client('http://mmsservice.tongkni.co.kr/mms.1/ServiceMMS.asmx?WSDL',true);
                $parameters[] = array(
                        'mmsID'=>$mms_id,
                        'hashValue'=>$hash_value
	...
	function ReserveCancle($mms_id,$mms_pwd,$userdefine)
        {
                $hash_value=md5($mms_id.$mms_pwd.$userdefine);
                $soapclient = new nusoap_client('http://mmsservice.tongkni.co.kr/mms.1/ServiceMMS.asmx?WSDL',true);
                $parameters[] = array(
                        'mmsID'=>$mms_id,
                        'hashValue'=>$hash_value,
