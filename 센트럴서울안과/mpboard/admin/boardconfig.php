	...
	<table class='admin_table_type2' summary='게시판 등록 - 리스트정보'>
		...
		<tbody>
			...
			<tr>
				...
				<th>관리팀</th>
				<td>
					<select name="manage_team" id="manage_team" class="input">
					<option value="" <?if($manage_team == "") echo "selected"?>>- 관리팀 지정 -</option>
					<option value="외래팀" <?if($manage_team == "외래팀") echo "selected"?>>외래팀</option>
					<option value="홍보팀" <?if($manage_team == "홍보팀") echo "selected"?>>홍보팀</option>
					<option value="미사용" <?if($manage_team == "미사용") echo "selected"?>>미사용</option>
					</select>
				</td>
			</tr>
			<tr>
				<th>NEW 표시</th>
				<td colspan='3'>
					<input name="new_time" type="text" class="text2" id="new_time" size="5" value="<?=$new_time?>">시간  (0 일경우 NEW 표시를 하지 않습니다.)
				</td>
			</tr>
		</tbody>
	</table>
