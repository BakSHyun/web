...
<div id='admin_sub_wrap'>
	...
	<p class='desc'><strong>[정렬방식 :</strong>
		<a href="<?=$PHP_SELF?>?m1=<?=$m1?>&amp;m2=<?=$m2?>&amp;orderby=st_cnt&amp;<?=$search_para?>"><? if($orderby == "st_cnt") { echo "<b class='red'>"; } ?>유입수 정렬<? if($orderby == "resdate") { echo "</b>"; } ?></a> /
		<a href="<?=$PHP_SELF?>?m1=<?=$m1?>&amp;m2=<?=$m2?>&amp;orderby=idx&amp;<?=$search_para?>"><? if($orderby == "idx") { echo "<b class='red'>"; } ?>등록순 정렬<? if($orderby == "wdate") { echo "</b>"; } ?></a>
	<strong>]</strong></p>

	<form name="cform"  method="post" action="<?=$PHP_SELF?>" onsubmit="return quedel(this)">
	<table class='admin_table_type1' style="color:black;" summary='관리자 게시판 리스트입니다.'>
		...
		$where = "where idx != '' ";
			...
			if($orderby == "st_cnt"){ $sqlorderby = "order by st_cnt desc";
			}else if($orderby == "idx"){ $sqlorderby = "order by idx desc"; }
			...
			$query = "select * from statistic_check $where $sqlorderby limit $sno, ".$pageobj->listnum." ";
			...
			<tr>
				...
				<td>
					<?
						if($row[st_n] == '다음' || $row[st_n] == '다음모바일'){
						echo $daum_ty ;
						} else if($row[st_n] == '네이버' || $row[st_n] == '네이버모바일'){
						echo $naver_ty;
						} else if($row[st_n] == '구글'){
						echo $google_ty;
						} else{
				...
				<td width='400px' style='word-break:break-all'>
					<?
						...
						if(strpos((string)$row[st_refer],"?")){
							$st_refer_strstr = explode("?",$row[st_refer]); 
							$st_refer_strstr = $st_refer_strstr[0];

							//$st_refer_strstr = strstr($row[st_refer], '?',true);
						}else{
							 $st_refer_strstr = $row[st_refer];
						}
						//$st_refer_strstr = strstr($row[st_refer], '?',true);
						if(strpos((string)$row[st_refer],"url")){
							$st_refer_urlstr = explode("url=",$row[st_refer]); 
							$st_refer_urlstr = $st_refer_urlstr[0];


							//$st_refer_urlstr = strstr($row[st_refer], 'url=',true);
						}else{
							 $st_refer_urlstr = $row[st_refer];
						}
						if($row[st_refer] == ''){
							echo "없음" ;
						}else{
							echo "<a target='_blank' href='".$st_refer_urlstr."'>".$st_refer_strstr."</a>";
						}
					?>
				</td>
				<td>
					<?	
						if(strpos((string)$row[st_url],"?")){
							$st_url_strstr = explode("?",$row[st_url]); 
							$st_url_strstr = $st_url_strstr[0];

							//$st_url_strstr = strstr($row[st_url], '?',true);
						}else{
							 $st_url_strstr = $row[st_url];
						}
						//$st_url_strstr = strstr($row[st_url], '?',true);
						if($st_url_strstr == ''){
							echo "<a target='_blank' href='".$row[st_url]."'>/</a>";
						}else{
							echo "<a target='_blank' href='".$row[st_url]."'>".$st_url_strstr."</a>";
						}
