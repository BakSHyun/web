...
require_once $_SERVER[DOCUMENT_ROOT]."/amconfig.php";
	...
	if(!isset($_COOKIE['st_chk'])){
		...
		if($_SERVER['HTTP_REFERER'] || $u_c){
			...
			$sqr[st_do]=$rf[host]; switch($sqr[st_do]){ case 'naver.com' : $sqr[st_n]='네이버'; break; case 'daum.net' : $sqr[st_n]='다음'; break; case 'nate.com' : $sqr[st_n]='다음'; break; case 'google.com' : $sqr[st_n]='구글'; break; case 'yahoo.com' : $sqr[st_n]='야후'; break; case 'zum.com' : $sqr[st_n]='줌'; break; default : $sqr[st_n]='기타'; break; }
		...
		$c_qry1=mysql_query("select count(idx) from statistic_check where st_n='".$sqr[st_n]."' and st_r_kw='".$sqr[st_r_kw]."' and st_kw='".$sqr[st_kw]."' and st_de='".$sqr[st_de]."' and st_url='".$sqr[st_url]."' ");
		...
		$pv_sql3=mysql_query("update statistic_check set st_cnt=st_cnt+1 where st_n='".$sqr[st_n]."' and st_r_kw='".$sqr[st_r_kw]."' and st_kw='".$sqr[st_kw]."' and st_de='".$sqr[st_de]."' ");
