...
<div id='admin_sub_wrap'>
	...
	<ul class='reserve_search_wrap2'>
		...
		<li class='reserve_search_wrap2_li'>
			<select name="search_cate1" class="select1">
				<option value="" >상담분야</option>
				<option value="임플란트" <? if($search_cate1 == "임플란트") { echo "selected"; } ?>>임플란트</option>
				<option value="치아교정" <? if($search_cate1 == "치아교정") { echo "selected"; } ?>>치아교정</option>
				<option value="심미치료" <? if($search_cate1 == "심미치료") { echo "selected"; } ?>>심미치료</option>
				<option value="충치치료" <? if($search_cate1 == "충치치료") { echo "selected"; } ?>>충치치료</option>
				<option value="잇몸치료" <? if($search_cate1 == "잇몸치료") { echo "selected"; } ?>>잇몸치료</option>
				<option value="신경치료" <? if($search_cate1 == "신경치료") { echo "selected"; } ?>>신경치료</option>
				<option value="수면치료" <? if($search_cate1 == "수면치료") { echo "selected"; } ?>>수면치료</option>
				<option value="기타치료" <? if($search_cate1 == "기타치료") { echo "selected"; } ?>>기타치료</option>
			</select>
		</li>
		<!-- <li class='reserve_search_wrap2_li'>
	...
	<table class='admin_table_type1' summary='관리자 게시판 리스트입니다.'>
		...
		$where = "where del_state!='Y' ";
			...
			if($search_cate1) $where .= "and cate1 = '$search_cate1' ";
...
if($code){
			...
			while($row=mysql_fetch_array($result)) {
				...
				if($row[rdate] ==""){
					$new="<img src='./img/new.gif' border='0' align='absmiddle'>";
				} else{
					$new="";
				}
				//echo $row[rdate];
			...
			<tr>
				...
				<?} else{?>
					<td><?=$row[position]?> <?=$row[etc10]?> / <?=$row[etc11]?></td>
				<? } ?>
				<? if($code =="event_friends"){?>
				...
				<?} else if($code =="event_new"){?>
					...
					<td><? if ($row[wmonth]){?><?=$row[wmonth]?>월 <?=$row[wday]?>일 <?=$row[whour]?> <?=$row[wmin]?><?}?></td>
					<td><?=$row[crm5]?></td>
					<td><?=$row[crm6]?></td>
				<?} else{?>
					<td><?=$row[etc1]?></td>
					...
					<? if($code <>"counsel"){?>
					<td><?=$row[cate1]?><?=$row[etc1]?></td>
					<? } ?>
					<td><a href="<?=$PHP_SELF?>?group=<?=$group?>&code=<?=$code?>&m1=<?=$m1?>&m2=crm_modify&no=<?=$row[no]?>&sno=<?=$sno?>&<?=$search_para?>" class="a"><?=$row[name]?></a>&nbsp;<?=$new?></td>
					<td><a href="<?=$PHP_SELF?>?group=<?=$group?>&code=<?=$code?>&m1=<?=$m1?>&m2=crm_modify&no=<?=$row[no]?>&sno=<?=$sno?>&<?=$search_para?>" class="a"><?=$row[hphone1]?>-<?=$row[hphone2]?>-<?=$row[hphone3]?></a></td>
					<td><? if ($row[wmonth]){?><?=$row[wmonth]?>월 <?=$row[wday]?>일 <?=$row[whour]?> <?=$row[wmin]?><?}?></td>
				<? } ?>
				<td><?=$row[etc3]?></td>
