<?
	...
	$st_qry1=mysql_query('select st_n, sum(st_cnt) from statistic_check where st_n != "기타" group by st_n');
	while($st_row1=mysql_fetch_array($st_qry1)){
		switch($st_row1[0]){
			case '네이버' : $cnt_naver=$st_row1[1]; break;
			case '네이버모바일' : $cnt_naver_m=$st_row1[1]; break;
			case '다음' : $cnt_daum=$st_row1[1]; break;
			case '다음모바일' : $cnt_daum_m=$st_row1[1]; break;
			case '구글' : $cnt_google=$st_row1[1]; break;
			case '줌' : $cnt_zum=$st_row1[1]; break;
			case '줌모바일' : $cnt_zum_m=$st_row1[1]; break;
			case '직접입력' : $cnt_dir=$st_row1[1]; break;
		}
	}
	$cnt_etc=$st_total[0]-$cnt_naver-$cnt_naver_m-$cnt_daum-$cnt_daum_m-$cnt_google-$cnt_zum-$cnt_zum_m-$cnt_dir;

	$st_row2=array();
	$st_qry22=mysql_query('select st_kw, sum(st_cnt) from statistic_check where st_kw != "직접입력" group by st_kw order by sum(st_cnt) desc limit 7');
	$ss=0;
...
//	for($i = 0; $i < count($m_stat) ; $i++){
//		foreach($m_stat as $key => $value){
//		echo $key." => ".$value[$i]."<br />";
//		}
//
//	}
  ...
  window.onload = function () {
    ...
    var chart4 = new CanvasJS.Chart("visit_portal",
    {
      ...
      data: [
      {
	   ...
	   <? if($cnt_naver_m){?> {  y: <?=$cnt_naver_m?>, legendText: "네이버모바일", indexLabel: "네이버모바일" }, <?}?>
       <? if($cnt_daum){?> {  y: <?=$cnt_daum?>, legendText: "다음", indexLabel: "다음" },<?}?>
	   <? if($cnt_daum_m){?> {  y: <?=$cnt_daum_m?>, legendText: "다음모바일", indexLabel: "다음모바일" },<?}?>
       <? if($cnt_google){?> {  y: <?=$cnt_google?>, legendText: "구글", indexLabel: "구글" },<?}?>
	   <? if($cnt_zum){?> {  y: <?=$cnt_zum?>, legendText: "줌", indexLabel: "줌" },<?}?>
	   <? if($cnt_zum_m){?> {  y: <?=$cnt_zum_m?>, legendText: "줌모바일", indexLabel: "줌모바일" },<?}?>
	...
	var chart3 = new CanvasJS.Chart("graph_month",
	{
		...
		data: [
		{
			...
			dataPoints: [
				{ label: '<?=$m_stat[0][0]?> 월', x: <?=$m_stat[0][0]?>, y: [<?=$m_stat[0][1]?>, <?=$m_stat[0][2]?>]},
				{ label: '<?=$m_stat[1][0]?> 월', x: <?=$m_stat[1][0]?>, y: [<?=$m_stat[1][1]?>, <?=$m_stat[1][2]?>]},
				{ label: '<?=$m_stat[2][0]?> 월', x: <?=$m_stat[2][0]?>, y: [<?=$m_stat[2][1]?>, <?=$m_stat[2][2]?>]},
				{ label: '<?=$m_stat[3][0]?> 월', x: <?=$m_stat[3][0]?>, y: [<?=$m_stat[3][1]?>, <?=$m_stat[3][2]?>]},
				{ label: '<?=$m_stat[4][0]?> 월', x: <?=$m_stat[4][0]?>, y: [<?=$m_stat[4][1]?>, <?=$m_stat[4][2]?>]},
				{ label: '<?=$m_stat[5][0]?> 월', x: <?=$m_stat[5][0]?>, y: [<?=$m_stat[5][1]?>, <?=$m_stat[5][2]?>]},
				{ label: '<?=$m_stat[6][0]?> 월', x: <?=$m_stat[6][0]?>, y: [<?=$m_stat[6][1]?>, <?=$m_stat[6][2]?>]},
				{ label: '<?=$m_stat[7][0]?> 월', x: <?=$m_stat[7][0]?>, y: [<?=$m_stat[7][1]?>, <?=$m_stat[7][2]?>]},
				{ label: '<?=$m_stat[8][0]?> 월', x: <?=$m_stat[8][0]?>, y: [<?=$m_stat[8][1]?>, <?=$m_stat[8][2]?>]},
				{ label: '<?=$m_stat[9][0]?> 월', x: <?=$m_stat[9][0]?>, y: [<?=$m_stat[9][1]?>, <?=$m_stat[9][2]?>]},
				{ label: '<?=$m_stat[10][0]?> 월', x: <?=$m_stat[10][0]?>, y: [<?=$m_stat[10][1]?>, <?=$m_stat[10][2]?>]},
				{ label: '<?=$m_stat[11][0]?> 월', x: <?=$m_stat[11][0]?>, y: [<?=$m_stat[11][1]?>, <?=$m_stat[11][2]?>]}
			]
		}
		]
	});
