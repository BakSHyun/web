...
$allow_image_max_width		= 800;

// 업로드제한용량(byte) (기본 : 1메가, 0: 무제한 , 서버의 설정에따라 별도의 제한이 걸릴 수 있습니다.)
$allow_image_max_volume		= 1024*1024*5;
...
if($_POST["proc_type"] == "quick_up") {
	...
	if(is_uploaded_file($_FILES["up_file"]["tmp_name"])) {
		...
		if($imageObj[2] > 0 && $imageObj[2] < 4) {
			...
			$filenamebody = str_replace(".","",$filenamebody);
...
//quick delete
	...
	$servefilepath = $upload_path."/".$servefilename;
        $servethumbfilepath = $upload_path."/t_".$servefilename;
	if(@unlink($servefilepath)) {
                @unlink($servethumbfilepath);
